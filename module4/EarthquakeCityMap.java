...
public class EarthquakeCityMap extends PApplet {
	...
	// To count the number of earthquakes that occurred in the particular country
	private int countryQuakes = 0;
	private int oceanQuakes = 0;
		...
		for (Marker country : countryMarkers) {
			for (Marker earthquake : quakeMarkers) {
				if (earthquake.getProperty("country") == country.getProperty("name")) {
					countryQuakes = countryQuakes + 1;
				}
			}
			if (countryQuakes > 0) {
				System.out.println(country.getProperty("name").toString() + ": " + countryQuakes);
			}
			countryQuakes = 0;
		}
